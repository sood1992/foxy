<?php
// bulk_scanner_v2.php - Enhanced Bulk Scanner with Optimized Ultra-Compact QR Support
...
<head>
    ...
    <style>
        ...
        /* Ultra-compact QR Toast */
        .ultra-compact-toast {
            background: linear-gradient(45deg, #FFD700, #FFF8DC) !important;
...
<body>
    ...
    <div class="main-container">
        ...
        <div class="page-header">
            ...
            <p class="text-muted">Ultra-compact QR scanning for CAM001 format - Perfect for 15mm labels</p>
        </div>
            ...
            <div class="step-body">
                ...
                <div class="method-content active" id="scanner-method">
                    <div class="scanner-area" id="scanner-area">
                        ...
                        <h5>Ultra-Compact QR Scanner</h5>
                        <p class="text-muted mb-3">Optimized for CAM001, AUD002 format - Perfect 15mm scanning</p>
                        <button class="btn btn-yellow" onclick="toggleScanner()" id="scanner-btn">
                            <i class="fas fa-play"></i> Start Scanner
                        </button>
                        
                        <!-- Camera Tips -->
                        <div class="camera-tips mt-3" style="display: none;" id="camera-tips">
                            <div class="alert alert-info py-2">
                                <div class="d-flex align-items-center">
                                    <i class="fas fa-lightbulb me-2"></i>
                                    <div style="font-size: 0.85rem;">
                                        <strong>📱 Camera Tips:</strong>
                                        <span id="device-specific-tip">Hold 4-6 inches from QR code for best results</span>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function showCameraTips() {
            ...
            const tipText = document.getElementById('device-specific-tip');
            
            if (tips && tipText) {
                // Detect device and show specific tips
                const userAgent = navigator.userAgent;
                let deviceTip = 'Hold 4-6 inches from QR code for best results';
                
                if (/iPhone|iPad|iPod/.test(userAgent)) {
                    deviceTip = 'iPhone: If wide-angle camera opens, use camera switch button or try different distances. Works best 4-6 inches away.';
                } else if (/Android/.test(userAgent)) {
                    deviceTip = 'Android: Make sure camera has permission to focus. Hold steady 4-6 inches from QR code.';
                } else if (/Chrome/.test(userAgent)) {
                    deviceTip = 'Desktop: QR codes work via webcam. Position QR code clearly in view.';
                }
                
                tipText.textContent = deviceTip;
        ...
        async function startScanner() {
            try {
                // Try to initialize camera with device preference
                await initializeCameraWithPreference();
                ...
                setTimeout(() => {
                    addCameraSwitchingHint();
                }, 2000);
                
            } catch (error) {
                console.error('Camera start error:', error);
                showToast('error', 'Camera access failed. Please allow camera permissions and try again.');
        ...
        function stopScanner() {
            if (html5Scanner) {
                ...
                // Remove any camera hints
                const hints = document.querySelectorAll('.camera-hint');
                hints.forEach(hint => {
                    if (hint.parentNode) {
        ...
        function addCameraSwitchingHint() {
            const qrReader = document.getElementById('qr-reader');
            if (!qrReader || !isScanning) return;
            
            // Check if we're likely on iPhone (common issue device)
            ...
            if (isIPhone) {
                ...
                `;
                hint.innerHTML = `
                    <i class="fas fa-mobile-alt"></i> iPhone Tip: 
                    If using wide-angle camera, tap the camera switch button in the scanner or try different distances
                `;
        ...
        async function initializeCameraWithPreference() {
            try {
                ...
                // For devices with multiple cameras, prefer the main camera
                if (videoDevices.length > 1) {
                    // iPhone usually lists: 
                    // 0: Front camera, 1: Main camera, 2: Ultra-wide, 3: Telephoto
                    // Android varies by manufacturer
                    
                    const preferredCameraIndex = findPreferredCamera(videoDevices);
                    if (preferredCameraIndex !== -1) {
                        return await startCameraWithDevice(videoDevices[preferredCameraIndex].deviceId);
                    }
                }
                
                // Fallback to default camera selection
                return await startCameraDefault();
        ...
        function findPreferredCamera(devices) {
            // Look for main/standard camera keywords (avoid ultra-wide)
            const preferredKeywords = ['main', 'standard', 'camera 1', 'back'];
            const avoidKeywords = ['ultra', 'wide', 'ultra-wide', 'fisheye'];
            ...
            for (let i = 0; i < devices.length; i++) {
                ...
                // Prefer main camera
                const hasPreferred = preferredKeywords.some(keyword => label.includes(keyword));
                const hasAvoided = avoidKeywords.some(keyword => label.includes(keyword));
                
                if (hasPreferred && !hasAvoided) {
                    return i;
                }
            }
            
            // If no preferred found, return first back camera
            for (let i = 0; i < devices.length; i++) {
        ...
        async function startCameraWithDevice(deviceId) {
            ...
            html5Scanner = new Html5QrcodeScanner("qr-reader", config);
            return html5Scanner.render(onScanSuccess, onScanError);
        }
        ...
        // OPTIMIZED QR SCANNING FUNCTION - Ultra-Compact CAM001 Format Priority
        function onScanSuccess(decodedText) {
            let assetId = null;
            
            console.log('QR Scanned:', decodedText);
            
            // Strategy 1: Clean format (CAM001, AUD002, etc.) - PRIMARY
            const cleanPattern = /^[A-Z]{3}\d{3}$/;
            if (cleanPattern.test(decodedText.trim())) {
                assetId = decodedText.trim();
                console.log('✅ Ultra-compact format detected:', assetId);
                
                if (!selectedItems.has(assetId)) {
                    addEquipment(assetId);
                    showToast('success', `Scanned: ${assetId} (Ultra-compact)`, true);
                } else {
            ...
            // Strategy 2: URL with clean asset_id (backward compatibility)
            const match = decodedText.match(/asset_id=([^&]+)/);
            if (match) {
                const extractedId = match[1];
                if (cleanPattern.test(extractedId)) {
                    assetId = extractedId;
                    console.log('✅ Clean ID from URL:', assetId);
                    
                    if (!selectedItems.has(assetId)) {
                        addEquipment(assetId);
                        showToast('success', `Scanned: ${assetId} (from URL)`);
                    } else {
                        showToast('warning', 'Equipment already added');
                    }
                    return;
                } else {
                    console.log('❌ Old format detected:', extractedId);
                    showToast('error', `Old format detected: "${extractedId}" - should have been migrated`);
                    return;
                }
            }
            
            // Strategy 3: Reject unrecognized formats
            console.log('❌ Unrecognized QR format:', decodedText);
            showToast('error', 'Invalid QR code - requires CAM001 format');
        }
        ...
        function addEquipment(assetId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Don't show toast here if it was already shown in onScanSuccess
                    if (!data.fromScan) {
        ...
        function showToast(type, message, isUltraCompact = false) {
            const toast = document.createElement('div');
            let bgClass = type === 'success' ? 'bg-success' : type === 'warning' ? 'bg-warning' : 'bg-danger';
            
            // Special styling for ultra-compact QR success
            if (isUltraCompact && type === 'success') {
                bgClass = 'ultra-compact-toast';
            }
